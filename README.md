...
**③ 나머지는 번역기 — 믿을 수 있는 것부터 차례로**

규칙으로 못 옮긴 문장만 번역기에 보냅니다. 번역기는 하나로 정하지 않고 **줄을 세웁니다.** 열쇠(API 키)를 넣어둔 정식 번역기를 먼저 쓰고, 없거나 실패하면 다음으로 내려갑니다.

| 순서 | 번역기 | 열쇠 | 왜 이 순서인가 |
|---|---|---|---|
| 1 | **DeepL** | 무료 가입, 월 50만 자 | 금융·법률 문장에 가장 강합니다 |
...
**어느 번역기가 옮겼는지 화면에 항상 표시됩니다** — `DeepL 번역`, `파파고 번역`, `무료 번역`. 어디까지 믿을지 직접 정하시라는 뜻입니다.

하나만 넣는다면 **DeepL**을 권합니다. 무료로 가입해서 열쇠를 받고, 환경변수나 `config.yml`에 넣으면 됩니다.
```yaml
translate:
  provider: "auto"      # 자동으로 가장 좋은 것 선택
  deepl_key: "..."      # 또는 환경변수 DEEPL_API_KEY
  allow_free: true      # 무료 경로를 못 믿겠으면 false
```
...
```
번역기마다 같은 문장을 보내보고 결과를 나란히 보여줍니다.

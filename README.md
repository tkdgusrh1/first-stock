...
| **화면은 뜨는데 종목이 0개, 검은 창에 `403 Forbidden`** | SEC가 접속을 거부한 것입니다. 원인은 대부분 **`user_agent` 에 한글이 들어간 것** — HTTP 헤더에 한글이 들어가면 차단됩니다. 지금은 봇이 자동으로 영문으로 바꿔 쓰지만, `config.yml` 의 `user_agent` 를 `"Gildong Hong hong@gmail.com"` 처럼 **영문 이름 + 이메일**로 적어두는 게 확실합니다. |

...
**③ 나머지는 기계 번역 — 그리고 그렇다고 밝힙니다**
규칙으로 못 옮긴 문장만 무료 번역으로 채우고 **`기계 번역`** 표시를 답니다. 자동 번역은 틀릴 수 있으니 원문을 함께 보시라는 뜻입니다. 한 번 받은 번역은 저장해두고 다시 받지 않습니다.

번역이 부담스러우면 `config.yml` 에서 끌 수 있습니다. 끄면 규칙으로 옮긴 한글만 나오고 나머지는 원문만 보입니다.
```yaml
translate:
  enabled: false
```
